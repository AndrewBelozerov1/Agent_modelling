...
def run_simulation_window_50(days=1000, noise_traders=50, chartists=20, fundamentalists=30):
    """Симуляция с окном скользящей средней = 50 дней (вместо 20)"""
    asset = Asset()
    ...
    # Чартисты с окном 50 дней (длинный горизонт)
    for _ in range(chartists):
        traders.append(ChartistTrader(cash=150000, assets=1000, window=50, alpha=0.1))
    ...
    _print_analysis(traders, initial_wealths, asset.price_history, 
                    noise_traders, chartists, fundamentalists, window_size=50)
...
def _log_status(day, asset, traders, noise_traders, chartists, fundamentalists):
    ...
    print(f"Active Traders - Noise: {noise_stats['active']}/{noise_traders}, "
          f"Chartists (window=50): {chart_stats['active']}/{chartists}, "
...
if __name__ == "__main__":
    print("ЗАПУСК ЭКСПЕРИМЕНТА: ОКНО СКОЛЬЗЯЩЕЙ СРЕДНЕЙ = 50 ДНЕЙ")
    
    asset, traders = run_simulation_window_50(days=1000)
    ...
    axes[0, 0].set_title('Цена актива (window=50)', fontsize=12)
    ...
    axes[1, 0].set_title(f'Среднее богатство технических трейдеров (window=50)', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig('../results/window_50_analysis.png', dpi=150)
    plt.show()
    ...
    axes[1].set_title(f'Распределение богатства\nтехнических трейдеров (window=50)', fontsize=12)
